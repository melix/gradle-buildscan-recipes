...
The recipe works by loading [a Java agent](http://file-leak-detector.kohsuke.org/) that will track file handles. If leaking files
...
resulting [build scan](https://scans.gradle.com/s/dpvxomrln43bo/custom-values). The details file contains the stack trace to
